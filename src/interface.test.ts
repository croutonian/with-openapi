...
import { OpenAPIInterface } from './index.js'
...
describe('OpenAPIInterface', () => {
  ...
  it('builds a middleware bound to that document', async () => {
    const handler = pipeline([api.middleware()], async () =>
      Response.json({ fell: true }),
    )
  ...
  it('passes the rest of the config through', async () => {
    const handler = pipeline(
      [api.middleware({ onUnknownRoute: 'pass' })],
  ...
  it('hands back the params of the operation that ran', async () => {
    const handler = pipeline([api.middleware()], async (_req, ctx) => {
  ...
  it('returns undefined when the request is a different route', async () => {
    const handler = pipeline([api.middleware()], async (_req, ctx) =>
      Response.json({ params: api.params(ctx, '/users/{id}', 'get') ?? null }),
    )
